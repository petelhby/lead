// src/app/(app)/projects/ClientProjectsPage.tsx
"use client";
...
function labelFromValue(v: any): string | null {
    ...
    if (typeof v === "object") return v.name || v.fullName || v.title || v.label || v.email || null;
    return null;
}

function extractAssigneesShallow(t: TaskAny): string[] {
    const out: string[] = [];
    const explicit = ["assignedTo", "assigned_to", "assignedUser", "assigned_user"] as const;
...
export default function ClientProjectsPage() {
    ...
    useEffect(() => {
        ...
        api("/api/projects", { auth: true }).then(setProjects).catch(() => setProjects([]));

        const fetchTasks = () => {
            const url = amAdmin ? "/api/tasks/all" : "/api/tasks/my";
            return api(url, { auth: true }).then(setTasks).catch(() => setTasks([]));
        };
    ...
    // ПРОЕКТЫ РАБОТНИКА (по его задачам)
    const workerProjectIds = useMemo(() => {
        ...
        for (const t of tasks) {
            ...
            if (Number(assignedId) === Number(me.id)) set.add(pid);
        }
        return set;
    }, [amAdmin, me?.id, tasks]);
        ...
        <div className="space-y-6">
                            ...
                            return (
                                <tr key={p.id} className="hover:bg-slate-50/50">
                                    <td className="px-5 py-4">
                                        <Link href={`/projects/${p.id}`} className="font-medium text-slate-900 hover:underline hover:text-[#0160C9]">
                                            {p.name}
                                        </Link>
                                    </td>
                                    ...
                                    <td className="px-5 py-4">
                                        <div className="flex flex-wrap gap-2">
                                            ...
                                            {assignees.map((name) => (
                                                <span key={name} className="inline-flex items-center rounded-md bg-[#d5c8be] px-2 py-1 text-xs text-[#3b2f2a]" title={name}>
                                                    {name}
                                                </span>
                                            ))}
                                        </div>
                                    ...
                                    <td className="px-5 py-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {amAdmin && (
                                                <Link
                                                    href={`/projects/${p.id}/finances`}
                                                    className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[#dcffd3] hover:bg-emerald-200"
                                                    title="Финансы"
                                                >
                                                    <SvgMask src="/ruble.svg" size={18} color="#000000" />
                                                </Link>
                                            )}
            ...
            {amAdmin && showEdit && editingProject && (
                <EditProjectModal
                    ...
                    onUpdated={(updated) => handleProjectUpdated(updated)}
                    onDeleted={(deletedId) => {
